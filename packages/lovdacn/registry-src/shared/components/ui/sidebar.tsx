...
function SidebarProvider({
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <View
        className={cn(
          "bg-background relative w-full flex-1 flex-row overflow-hidden",
          className,
        )}
        {...props}
      >
...
function Sidebar({
  ...
  if (isMobile) {
    return (
      <SidebarDrawer side={side} open={openMobile} onOpenChange={setOpenMobile}>
        <View
          className={cn("bg-sidebar h-full w-full flex-col", className)}
          {...props}
        >
          {children}
        </View>
  ...
  return (
    <SidebarDesktop
      width={targetWidth}
      floating={variant === "floating" || variant === "inset"}
    >
      <View
        className={cn(
...
function SidebarDesktop({
  ...
  return (
    <Animated.View style={{ width: animated, overflow: "hidden" }}>
      <View
        className={cn("h-full w-full", floating && "p-2")}
        style={{ width: floating ? width : undefined }}
      >
        {children}
      </View>
    </Animated.View>
...
function SidebarDrawer({
  ...
  const drawerWidth = Math.min(
    ...
    Math.max(0, viewportWidth - insets.left - insets.right - 16),
  );
    ...
    >
      ...
      <Animated.View
        style={{
          ...
          left: side === "left" ? insets.left : undefined,
          right: side === "right" ? insets.right : undefined,
          width: drawerWidth,
...
function SidebarTrigger({
  ...
  return (
    <Pressable
      ...
      className={cn(
        "h-11 min-h-11 w-11 min-w-11 items-center justify-center rounded-md",
  ...
  return (
    <Pressable
      ...
      className={cn(
        ...
        Platform.select({ web: "hover:bg-sidebar-border" }),
        className,
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Text
      className={cn(
        "text-sidebar-foreground/70 h-8 px-2 py-1.5 text-xs font-medium",
        className,
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "group min-h-11 min-w-11 w-full flex-row items-center gap-2 overflow-hidden rounded-md px-2",
  {
    variants: {
      variant: {
...
function SidebarMenuButton({
  ...
  const content =
    collapsed && typeof children !== "function"
    ...
    >
      <Pressable
        ...
        className={cn(
          ...
          collapsed &&
            "h-8 min-h-11 w-8 min-w-11 justify-center gap-0 self-center px-0",
          Platform.select({
            web: !isActive ? "active:bg-sidebar-accent" : undefined,
          }),
          className,
        )}
        {...props}
...
function SidebarMenuSkeleton({
  ...
  if (collapsed) {
    return (
      <View
        className={cn(
          "size-8 items-center justify-center self-center rounded-md",
          className,
        )}
        {...props}
      >
...
function SidebarMenuSub({
  ...
  return (
    <View
      className={cn(
        "border-sidebar-border ml-3.5 flex-col gap-1 border-l px-2.5 py-0.5",
        className,
      )}
      {...props}
    />
