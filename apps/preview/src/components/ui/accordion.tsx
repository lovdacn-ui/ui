...
import { durations, Pressable, View } from '@preview/components/ui/primitives';
...
function AccordionItem({
  ...
  return (
    <AccordionPrimitive.Item
      className={cn(
        'border-border border-b not-last:border-b',
    ...
    <TextClassContext.Provider
      ...
      <AccordionPrimitive.Header>
        <AccordionPrimitive.Trigger {...props} asChild>
          <Trigger
            className={cn(
              'flex-row items-start justify-between rounded-md disabled:opacity-50 gap-6 p-4 text-left text-sm font-medium hover:underline',
              Platform.select({
